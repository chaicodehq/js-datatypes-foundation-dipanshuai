...
export function parseFare(fareString) {
  ...
  const parsedFloat = parseFloat(fareString)
  if (Number.isNaN(parsedFloat)) return -1
...
export function roundFare(amount, decimalPlaces) {
  ...
  if (typeof (decimalPlaces) != "number" || decimalPlaces < 0 || Number.isInteger(decimalPlaces) === false) return ""
...
export function getDistanceDifference(from, to) {
  ...
  if (Number.isNaN(newFrom) || Number.isNaN(newTo)) return -1
